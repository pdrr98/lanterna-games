...
public class Tutorial03 {
    public void run() {
         ...
         * 
         * A Screen works similar to double-buffered video memory, it has two surfaces
         * than can be directly addressed and
         * modified and by calling a special method that content of the back-buffer is
         * move to the front. Instead of pixels
         * though, a Screen holds two text character surfaces (front and back) which
         * corresponds to each "cell" in the
         * terminal. You can freely modify the back "buffer" and you can read from the
         * front "buffer", calling the
         * refreshScreen() method to copy content from the back buffer to the front
         * buffer, which will make Lanterna also
        ...
        try {
             ...
             * generally give you the TerminalScreen
             * implementation that is probably what you want to use. Please see
             * VirtualScreen for more details on a separate
             * implementation that allows you to create a terminal surface that is bigger
             * than the physical size of the
             * terminal emulator the software is running in. Just to demonstrate that a
             * Screen sits on top of a Terminal,
             ...
             * mutate its state, before you can
             * make any of these changes visible, you'll need to call startScreen() which
             * will prepare and setup the
             * terminal.
             */
            screen.startScreen();
             ...
             * is visible yet. In order to
             * move the content from the back buffer to the front buffer and refresh the
             ...
             * supports colors). Let's look at it for two seconds or until the user press a
             * key.
             */
             ...
             * pressing escape on the keyboard or the
             * input stream is closed. When using the Swing/AWT bundled emulator, if the
             * user closes the window this will
            ...
            while (true) {
                 ...
                 * let the Screen know when is
                 * a good time to update its internal buffers. Usually you should do this at the
                 * start of your "drawing"
                 * loop, if you have one. This ensures that the dimensions of the buffers stays
                 * constant and doesn't change
                 * while you are drawing content. The method doReizeIfNecessary() will check if
                 * the terminal has been
                 * resized since last time it was called (or since the screen was created if
                 * this is the first time
                 * calling) and update the buffer dimensions accordingly. It returns null if the
                ...
                for (int i = 0; i < charactersToModifyPerLoop; i++) {
                     ...
                     * TerminalSize, TextCharacter
                     * objects are immutable so the withBackgroundColor(..) call below returns a
                     * copy with the background color
                     * modified.
                     */
